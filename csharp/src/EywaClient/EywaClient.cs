...
/// Uses JsonNode for all data interchange to stay as close to GraphQL
/// as possible while providing natural indexer syntax for RPA/microservices.
/// </summary>
...
public class Eywa : IDisposable
{
    ...
    /// </summary>
    public async Task<JsonNode?> GraphQLAsync(string query, object? variables = null)
    {
        ...
        if (result?["error"] != null)
        {
            ...
            throw new GraphQLException($"GraphQL error: {error?.ToJsonString()}");
        }
